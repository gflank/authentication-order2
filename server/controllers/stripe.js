...
exports.charge = function (req, res, next) {
  ...
  Order.findById(orderId, (err, order) => {
    ...
    if (order.userId.toString() != req.user._id.toString()) {
      return res.status(401).json({ error: 'You are not authorized to pay this order.' });
    }
    ...
    var charge = stripe.charge.create(payment, (err, charge) => {
      if (err) {
        switch (err.type) {
...
exports.createCustomer = function (req, res, next) {
  ...
  User.findById(req.user._id, (err, user) => {
    ...
    if (user.stripe.customerId) {
      return res.status(500).json({ error: 'The customer exists' });
      next(err);
    }
    ...
    stripe.customers.create({
      ...
      user.save((err, updatedUser) => {
        ...
        return res.status(200).send({ message: 'Payment method successfully added.' });
      });
    }).catch(err => next(err));
  });
};
